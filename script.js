...
const flagChange = (element) => {
  let countCurrency = element.value;
  let countryCode = countryList[countCurrency];
...
const updateRate = async () => {
  ...
  let from_Currency = fromCurrency.value.toLowerCase();
  let to_Currency = toCurrency.value.toLowerCase();

  let rate = data[from_Currency][to_Currency];

  let finaleAmount = amountVal * rate;

  let msg = document.querySelector(".msg");
  msg.innerText = `${amountVal}${fromCurrency.value} = ${finaleAmount}${toCurrency.value}`;
